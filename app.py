...
# Deliberately a SEPARATE value from SHOPIFY_APP_CLIENT_ID above — this is
# the theme app extension's own UUID, generated by Shopify only once
# `shopify app deploy` actually runs, retrievable afterward via
# `shopify app info`. Using the app's client_id here (as an earlier
# version of this code did) produces Shopify's "App embed does not exist"
# error — the deep link's activateAppId parameter needs the EXTENSION's
# identity, not the APP's. See _build_activate_embed_url's docstring.
SHOPIFY_EXTENSION_UUID    = os.environ.get('SHOPIFY_EXTENSION_UUID', '')
...
def connect_shopify_callback():
    ...
    #
    # {handle} is the liquid filename (chat-embed.liquid -> "chat-embed").
    # The first part of activateAppId is NOT SHOPIFY_APP_CLIENT_ID — an
    # earlier version of this code used the app's client_id here, which
    # produces Shopify's "App embed does not exist" error. It needs the
    # extension's OWN UUID instead, a separate identifier Shopify only
    # generates once `shopify app deploy` actually runs (retrievable
    # afterward via `shopify app info`) — see SHOPIFY_EXTENSION_UUID above.
    #
    # Fails safe: if that UUID isn't configured yet (extension never
    # deployed, or deployed but the UUID wasn't copied into this app's
    # environment), omit the activation link entirely rather than send
    # merchants to a URL that's guaranteed to show the same error this
    # comment is describing. The merchant still successfully connected —
    # they just won't see a broken "Activate" button while this is
    # unresolved on the deploy side.
    activate_embed_url = None
    if SHOPIFY_EXTENSION_UUID:
        activate_embed_url = (
            f'https://{shop}/admin/themes/current/editor'
            f'?context=apps&template=index&activateAppId={SHOPIFY_EXTENSION_UUID}/lumvi-chat-embed'
        )
    else:
        app.logger.warning('[Shopify OAuth] SHOPIFY_EXTENSION_UUID not set — omitting '
                           'activate_embed_url (would produce "App embed does not exist")')

    if return_to == 'dashboard':
        return redirect(url_for('auth.dashboard', connected=1, activate_embed_url=activate_embed_url))
    if return_to == 'integrations':
        return redirect(url_for('integrations_page', client_id=client_id, connected=1, activate_embed_url=activate_embed_url))
    return redirect(url_for('auth.onboarding', step=2, connected=1, client_id=client_id, activate_embed_url=activate_embed_url))
